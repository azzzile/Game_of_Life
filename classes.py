...
class Field:
    ...
    def __init__(self, width, height, colors, cell_size):
    ...
    def set_boarders(self, boarders):
        """устанавливаем отступы"""
        self.boarders = boarders

    def next_step(self, screen, ongoing):
        ...
        for j in range(self.height):
            for i in range(self.width):
                ...
                pygame.draw.rect(screen, cur.color,
                                 (self.boarders[0] + i * self.cell_size, self.boarders[1] + j * self.cell_size,
                                  -self.cell_size, -self.cell_size))  # рисуем саму клетку
        ...
        print([x.is_alive() for x in self.field[1]])
